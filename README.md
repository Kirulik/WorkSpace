...
## HTML-справка с поиском (MkDocs Material)

Статический сайт с централизованным поиском по всем разделам — для просмотра и последующего встраивания в веб-UI конструктора плагинов:

```bash
pip install -r requirements.txt
mkdocs serve          # http://127.0.0.1:8000
mkdocs build          # готовый пакет в site/
```

После `mkdocs build` папка `site/` содержит самодостаточный HTML/CSS/JS (индекс поиска уже внутри).
...
Маркер **⚠ ТРЕБУЕТСЯ ДОПОЛНЕНИЕ** означает, что в материалах (`functions.txt`, исходники, ODT) нет достаточного текстового описания — место отмечено для ручного дополнения тимлида. Самостоятельные домыслы в справку не включались.
