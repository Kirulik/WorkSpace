...
Справочник по встроенному скриптовому API платформы **UniServer AUTO** (PascalScript / FastScript).

Документ построен по структуре `functions.txt`. Описания функций приведены только на основании `functions.txt`, исходников (`fsCommon.pas`, `fsCoreScript.pas`, `fsJsonVar.pas`, `PluginAPI_TLB.pas`) и ODT-материалов. Неподтверждённые сведения не добавлялись.
...
Откройте нужный раздел в блоке **Содержание** (раскрывающиеся списки выше).
