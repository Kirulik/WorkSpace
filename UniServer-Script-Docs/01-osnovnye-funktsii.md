...
Возвращает значение `V` как `Double`. Удобно безопасно привести Variant или строку с числом к вещественному типу.
...
Возвращает новую строку packed GUID. Используется как компактный идентификатор записи, документа или операции.
...
Возвращает `True`, если строка `Guid` имеет формат packed GUID. Позволяет отличить «живой» код связи или записи от пустого или произвольного текста.
...
Приостанавливает выполнение скрипта на заданное число миллисекунд (значение ограничивается диапазоном 0…10000).
...
Возвращает `True`, если значение считается пустым. Для JSON-массива/объекта смотрит на `Count = 0`, для строк — на нулевую длину, иначе опирается на `VarIsEmptyOrNull`.
...
Возвращает `True`, если Variant пуст или равен `Null`. Базовая проверка «нет данных» перед работой со значением.
...
Пишет переданное значение в лог-файл плагина (`EventScript.log`) для отладки сценария.
